...
"""
Best score:
+['f_41] + 1000 estimators = 437
+['f_17] + 1000 estimators = 435
 simple + 100 estimators   = 428


"""

clf = ExtraTreesClassifier(max_features=0.4, criterion='entropy', n_estimators=1000, n_jobs=4)
eclf = EnsembleVoteClassifier(clfs=[clf], weights=[1], voting='soft')
...
for iteration in range(10):
    ...
    for feature in good_features_drop_no_corr:
        ...
        print('Current feature [%s] = %s' % (feature, correct_answers))

        if correct_answers > best_score:
            best_feature = feature
            best_score = correct_answers

    print('Best feature:', best_feature, best_score, best_columns)
    best_columns.append(best_feature)
