...
def hyperopt_train_test(hpparams):
    ...
    params_est = {
        ...
        'splitter': hpparams['splitter'],
        'max_features': hpparams['max_features'],
        'min_samples_split': round(hpparams['min_samples_split']),
        'min_samples_leaf': 1,
    }

    pipeline = DecisionTreeClassifier(**params_est)
...
space4dt = {
   ...
   'min_samples_split': hp.uniform('min_samples_split', 50, 5000),
...
def f(params):
    ...
    best_params.to_csv('results/search_df_14_05_best_features_DECISIONTREE.csv', index=False)
